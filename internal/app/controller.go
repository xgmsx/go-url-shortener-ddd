...
func getHTTPController(ch chan error, c *config.Config, uc *usecase.UseCase) *http.Server {
	metricsConfig := metrics.Config{ServiceName: c.App.Name, Registry: prometheus.DefaultRegisterer}
	tracesConfig := traces.Config{ServerName: c.App.Name, CollectClientIP: true}
	tracesConfig.SetSkipPaths("/metrics", "/live", "/ready")
	...
	return http.New(ch, c.HTTP, http.BuildOptions(c.HTTP,
		...
		http.WithMiddleware(traces.New(tracesConfig), c.Otel.Endpoint != ""),
		http.WithMiddleware(metrics.New(metricsConfig)),
